"""pipeline_workbench.py -- V2 workbench for the Tableau to SiS accelerator.

A SEPARATE review entry point. It does not import or modify pipeline_app.py,
and it is not referenced by snowflake.yml. The deployed staged demo is
untouched until this build is explicitly approved.

Design follows tableau_to_sis_v2_preview.html and the approved
pipeline_app_v2.py prototype. All migration logic is reused from the existing
modules (pipeline / tableau_parser / semantic_layer / cortex_semantic /
codegen / engine / parity) -- nothing is reimplemented here.

    streamlit run pipeline_workbench.py --server.port 8520
"""
...
def sidebar(session, session_label):
    with st.sidebar:
        ...
        st.markdown('<div class="v2-sidebar-note">Separate V2 review build. '
                    'The deployed staged demo is unchanged.</div>', unsafe_allow_html=True)
    return page
...
def page_architecture():
    render_header("Platform architecture",
                  "How the accelerator is wired: external systems in the margins, "
                  "layers in the middle, Cortex called in-account.")
    try:
    ...
    st.caption("Rendered from gen_platform_architecture.py — the same source used for the "
               "standalone architecture page, so this view can never drift from it.")
...
def render_tableau_intake():
    ...
    site_url = st.text_input("Tableau site URL", key="v2_site_url",
...
def run_migration(source, session, replicate_model=False, emit=None):
    ...
    if "CREATE OR REPLACE SEMANTIC VIEW" in semantic_ddl and metrics:
        ...
        if in_sf:
            try:
                if pipeline.semantic_view_exists(session, semantic_view):
                    semantic_state = "reused"
                else:
                    session.sql(semantic_ddl).collect()
                    semantic_state = "deployed"
            except Exception as exc:
...
def render_dashboard_preview(ir):
    ...
    selected = st.selectbox("Dashboard", titles, key="v2_dashboard")
    dashboard = next(d for d in dashboards if d["title"] == selected)
    try:
        engine.render_dashboard(dashboard)
    except Exception as exc:
        st.error(f"Dashboard could not render: {type(exc).__name__}: {exc}")
        with st.expander("Error details"):
            st.code(traceback.format_exc())
    notes = findings.all_findings()
    if notes:
        with st.expander(f"Migration notes for this render ({len(notes)})"):
...
def page_deploy(session):
    ...
    with right:
        ...
        if state not in ("deployed", "reused"):
