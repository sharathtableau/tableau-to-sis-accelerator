...
<!-- Slide number: 8 -->
BLEND360
NEXT STEPS · 08  ·  CORE PRESENTATION ENDS HERE
...
### Notes:
Use if a Snowflake engineer asks how the pipeline actually works end to end. Walk left to right: ingestion/discovery, conversion into an IR, Cortex called from inside the account (proposes only, never final say), semantic and serving layer, then the three-lane QA gate (data / calc / visual) before anything ships.
